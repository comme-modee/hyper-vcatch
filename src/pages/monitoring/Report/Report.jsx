...
const Report = () => {
  ...
  useEffect(()=>{
    ...
    if(client) {
      clientListData.forEach(list => list.client_uid === parseInt(client, 10) ? setClientName(list.client_name) : '')
    }
    ...
    if(client && daytime) {
      if(currentWeek || selectedWeek || (startDate && endDate) || (selectedYear && selectedMonth)){
        handleOnExport({ reportType, client, clientName, daytime, currentWeek, selectedWeek, startDate, endDate, month:selectedMonth, year:selectedYear })
        setDaytime('')
      } 
    }
	...
	},[userInfo, selectedDate, client, daytime, selectedStartDate, selectedEndDate, startDate, endDate, reportType, currentWeek, selectedWeek, selectedYear, selectedMonth, showClient, clientName ])
						...
						<Col xl={5} lg={7} md={7} sm={9} xs={9}>
							<div className="horizontal-steps mt-4 mb-4 pb-5" >
								<div className="horizontal-steps-content">
									<div className={`step-item ${!showClient ? 'current' : ''}`} style={{whiteSpace: 'nowrap'}}>
											<span>기간 선택</span>
									</div>
									<div className={`step-item ${showClient && client == '' ? 'current' : ''}`} style={{whiteSpace: 'nowrap'}}>
											<span>클라이언트 선택</span>
									</div>
									<div className={`step-item ${showClient && client !== '' ? 'current' : ''}`} style={{whiteSpace: 'nowrap'}}>
										<span>보고서 완성</span>
								...
								<div className="process-line report-process-line" style={{ width: showClient && client !== '' ? '100%' : showClient ? '50%' : '0%' }}></div>
							</div>
						</Col>
				</Row>
        <div className='pe-2 ps-2 position-relative z-2'>
          <Row className="justify-content-center">
              ...
              </Col>
              {
                      ...
                      : 
                      <Col xl={4} lg={4} md={4} sm={4} xs={9} className='mb-3'>
